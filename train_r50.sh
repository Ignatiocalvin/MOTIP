...
#   NUM_CONCEPTS: 0 (base), 2 (2concepts), 3 (3concepts), 7 (7concepts)
...
NUM_CONCEPTS=2               # 0=base, 2=2concepts, 3=3concepts, 7=7concepts
...
case $NUM_CONCEPTS in
    ...
    7)
        if [ "$USE_LEARNABLE_WEIGHTS" = "true" ]; then
            ...
            CONFIG="./configs/r50_motip_pdestre_7concepts_nolw.yaml"
            EXP_NAME="r50_motip_pdestre_7concepts_nolw_fold_${FOLD}"
        fi
        ;;
    *)
...
if [ -n "$SLURM_JOB_ID" ]; then
    ...
    JOB_LABEL="motip_r50_${NUM_CONCEPTS}c${LW_SUFFIX}_f${FOLD}"
    scontrol update JobId="$SLURM_JOB_ID" JobName="$JOB_LABEL" 2>/dev/null || true
    mkdir -p logs
    ln -sf "motip_r50_${SLURM_JOB_ID}.out" "logs/${JOB_LABEL}_${SLURM_JOB_ID}.out" 2>/dev/null || true
    ln -sf "motip_r50_${SLURM_JOB_ID}.err" "logs/${JOB_LABEL}_${SLURM_JOB_ID}.err" 2>/dev/null || true
fi
