...
def main():
    ...
    for (exp_folder, val_split, label) in EXPERIMENTS:
        ...
        print(f"{'='*60}")
        print(f"  {'Epoch':>6}  {'MOTA':>7}  {'IDF1':>7}  {'Prec':>7}  {'Rec':>7}  {'IDSW':>6}  {'TP':>7}  {'FN':>7}  {'FP':>7}")
        print(f"  {'-'*6}  {'-'*7}  {'-'*7}  {'-'*7}  {'-'*7}  {'-'*6}  {'-'*7}  {'-'*7}  {'-'*7}")
        ...
        for ep_dir in epoch_dirs:
            ...
            try:
                ...
                if cache_key in cache:
                    m = cache[cache_key]
                    print(f"  {ep_num:>6}  {fmt(m['mota']):>7}  {fmt(m['idf1']):>7}  "
                          f"{fmt(m['prec']):>7}  {fmt(m['rec']):>7}  {m['idsw']:>6}  "
                          f"{m['tp']:>7}  {m['fn']:>7}  {m['fp']:>7}  (cached)")
                else:
        ...
        if rows:
            print(f"\n  ★ Best: Epoch {best_ep}  MOTA={best_mota:.2f}%")
    ...
    for (label, exp_folder, val_split, rows) in pdestre_sections:
        ...
        lines.append("")
        lines.append("| Epoch | MOTA (%) | IDF1 (%) | Prec (%) | Rec (%) | IDSW | TP | FN | FP |")
        lines.append("|------:|---------:|---------:|---------:|--------:|-----:|---:|---:|---:|")
        ...
        if rows:
            lines.append("")
            lines.append(f"**★ Best: Epoch {best_ep} — MOTA = {best_mota:.2f}%**")
        else:
            lines.append("")
