#!/bin/bash
#SBATCH --job-name=motip_rfdetr_7c_resume
...
#SBATCH --mem=16G
#SBATCH --output=logs/motip_rfdetr_7c_resume_%j.out
#SBATCH --error=logs/motip_rfdetr_7c_resume_%j.err
...
if [ -z "$SLURM_JOB_ID" ]; then
    mkdir -p logs
    LOG_FILE="logs/motip_rfdetr_7c_resume_$(date +%Y%m%d_%H%M%S).log"
    echo "Logging to: $LOG_FILE"
    exec > >(tee -a "$LOG_FILE") 2>&1
fi

echo "=========================================="
echo "MOTIP RF-DETR Large 7-Concept (NO learnable weights) - Resume from checkpoint_2"
...
echo ""
echo "Resuming: rfdetr_large_motip_pdestre_7concepts_learnable_fold0 (checkpoint_2)"
...
python -u -m accelerate.commands.launch --num_processes=1 train.py \
    ...
    --resume-model ./outputs/rfdetr_large_motip_pdestre_7concepts_learnable_fold0/checkpoint_2.pth \
