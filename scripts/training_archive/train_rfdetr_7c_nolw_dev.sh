#!/bin/bash
#SBATCH --job-name=motip_7c_nolw_dev
...
#SBATCH --output=logs/motip_7c_nolw_dev_%j.out
#SBATCH --error=logs/motip_7c_nolw_dev_%j.err
#SBATCH --exclude=uc3n073
#SBATCH --chdir=/pfs/work9/workspace/scratch/ma_ighidaya-thesis_ignatio/MOTIP

# ── identity ──────────────────────────────────────────────────────────────────
EXP_NAME="rfdetr_large_motip_pdestre_7concepts_learnable_fold0"
CONFIG="./configs/rfdetr_large_motip_pdestre_7concepts_nolw_fold0.yaml"
INITIAL_CKPT="./outputs/${EXP_NAME}/checkpoint_2.pth"
SCRIPT_PATH="/pfs/work9/workspace/scratch/ma_ighidaya-thesis_ignatio/MOTIP/train_rfdetr_7c_nolw_dev.sh"
...
# First run: resume from checkpoint_2.pth (the canceled training).
# Subsequent runs: use latest intra-epoch checkpoint automatically.
OUTPUTS_DIR="./outputs/${EXP_NAME}"
INTRA_DIR="${OUTPUTS_DIR}/intra_epoch_checkpoints"
RESUME_ARGS=()
if ls "${INTRA_DIR}"/*.pth 2>/dev/null | head -1 | grep -q .; then
    echo "[checkpoint] Intra-epoch checkpoints found — resuming from latest"
    RESUME_ARGS=(--use-previous-checkpoint True)
elif [ -f "$INITIAL_CKPT" ]; then
    echo "[checkpoint] No intra-epoch checkpoints — using initial checkpoint: $INITIAL_CKPT"
    RESUME_ARGS=(--resume-model "$INITIAL_CKPT")
else
    echo "[checkpoint] WARNING: $INITIAL_CKPT not found — starting fresh"
fi
