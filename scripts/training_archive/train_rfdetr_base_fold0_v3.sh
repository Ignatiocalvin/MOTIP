#!/bin/bash
#SBATCH --job-name=motip_rfdetr_base_v3
...
#SBATCH --mem=16G
#SBATCH --output=logs/motip_rfdetr_base_v3_%j.out
#SBATCH --error=logs/motip_rfdetr_base_v3_%j.err
...
echo "=========================================="
echo "RF-DETR Large MOTIP P-DESTRE Base (v3) - Fold 0"
echo "v3 adds PRETRAIN_WEIGHTS=rf-detr-large.pth for fair comparison with 7C models."
echo "Only 2 shape-compatible keys load (bbox head biases); handled by strict=False."
...
python -u -m accelerate.commands.launch --num_processes=1 train.py \
    --data-root ./data/ \
    --exp-name rfdetr_large_motip_pdestre_base_fold0_v3 \
    --config-path ./configs/rfdetr_large_motip_pdestre_base_fold0_v3.yaml \
    2>&1
