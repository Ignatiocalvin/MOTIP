#!/bin/bash
#SBATCH --job-name=motip_3concepts
...
#SBATCH --mem=16G
#SBATCH --output=logs/motip_3concepts_%j.out
#SBATCH --error=logs/motip_3concepts_%j.err
...
if [ -z "$SLURM_JOB_ID" ]; then
    mkdir -p logs
    exec > >(tee -a "logs/motip_3concepts_$(date +%Y%m%d_%H%M%S).log") 2>&1
fi

echo "=========================================="
echo "MOTIP 3-Concept Resume (epoch_2_step_40000)"
...
accelerate launch --num_processes=1 train.py \
    --data-root ./data/ \
    --exp-name r50_motip_pdestre_3concepts_fold_0 \
    --config-path ./configs/r50_deformable_detr_motip_pdestre_3concepts.yaml \
