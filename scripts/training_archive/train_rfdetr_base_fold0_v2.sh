#!/bin/bash
#SBATCH --job-name=motip_rfdetr_base
...
#SBATCH --mem=16G
#SBATCH --output=logs/motip_rfdetr_base_%j.out
#SBATCH --error=logs/motip_rfdetr_base_%j.err
...
echo "=========================================="
echo "RF-DETR Large MOTIP P-DESTRE Base (v2) - Fold 0"
...
python -u -m accelerate.commands.launch --num_processes=1 train.py \
    --data-root ./data/ \
    --exp-name rfdetr_large_motip_pdestre_base_fold0_v2 \
    --config-path ./configs/rfdetr_large_motip_pdestre_base_fold0_v2.yaml \
    2>&1
