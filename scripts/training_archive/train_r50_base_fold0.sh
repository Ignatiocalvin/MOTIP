#!/bin/bash
#SBATCH --job-name=motip_r50_base
...
#SBATCH --mem=16G
#SBATCH --output=logs/motip_r50_base_%j.out
#SBATCH --error=logs/motip_r50_base_%j.err
...
if [ -z "$SLURM_JOB_ID" ]; then
    mkdir -p logs
    exec > >(tee -a "logs/motip_r50_base_$(date +%Y%m%d_%H%M%S).log") 2>&1
fi

echo "=========================================="
echo "MOTIP R50 Base (0 concepts) Fold 0"
...
accelerate launch --num_processes=1 train.py \
    --data-root ./data/ \
    --exp-name r50_motip_pdestre_base_fold0 \
