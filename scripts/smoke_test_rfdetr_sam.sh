...
#SBATCH --output=logs/smoke_test_rfdetr_sam_%j.out
#SBATCH --error=logs/smoke_test_rfdetr_sam_%j.err

########################################
# RF-DETR + SAM Concept Bottleneck Smoke Test
# Tests DanceTrack with SAM precomputed masks
########################################

echo "========================================"
echo "[RFDETR+SAM SMOKE TEST] Starting..."
echo "  Start: $(date)"
...
rm -rf outputs/smoke_test_rfdetr_sam

# Run smoke test training
echo ""
echo "[TRAIN] Starting RF-DETR + SAM smoke test training..."
python train.py \
    --config-path configs/smoke_test_rfdetr_sam.yaml \
    --exp-name smoke_test_rfdetr_sam

echo ""
echo "[DONE] RF-DETR + SAM smoke test completed!"
echo "  End: $(date)"
echo "========================================"

# Verify outputs
if [ -f "outputs/smoke_test_rfdetr_sam/checkpoint_1.pth" ]; then
    echo "[CHECK] ✅ Checkpoint found: outputs/smoke_test_rfdetr_sam/checkpoint_1.pth"
else
