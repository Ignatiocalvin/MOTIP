#!/bin/bash
#SBATCH --job-name=eval_r50_base_fold0
...
#SBATCH --mem=24G
#SBATCH --output=logs/eval_r50_base_fold0_%j.out
#SBATCH --error=logs/eval_r50_base_fold0_%j.err
...
EXP="r50_motip_pdestre_base_fold0"
...
output_dir = Path('outputs/r50_motip_pdestre_base_fold0')
...
if epoch_results:
    ...
    with open(f'outputs/r50_motip_pdestre_base_fold0/eval_results.json', 'w') as f:
        json.dump({'all_epochs': {str(k): v for k, v in epoch_results.items()}, 'best_epoch': best_ep, 'best': best}, f, indent=2)
    print(f"\nResults saved to outputs/r50_motip_pdestre_base_fold0/eval_results.json")
else:
